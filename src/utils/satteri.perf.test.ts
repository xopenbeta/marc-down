...
const TEST_FILE = '/Users/zws/Documents/mdTest/test_963k.md';

describe('satteri parse speed', () => {
  it('parses test_963k.md with markdownToMdast', () => {
    const source = readFileSync(TEST_FILE, 'utf-8');
    const bytes = Buffer.byteLength(source, 'utf-8');

    // warm up (WASI / module init)
    markdownToMdast(source, { features: { gfm: true } });
    ...
    for (let i = 0; i < runs; i++) {
      ...
      const tree = markdownToMdast(source, { features: { gfm: true } });
    ...
    console.log(
      [
        '',
        '===== satteri (markdownToMdast) parse speed =====',
        `file        : ${TEST_FILE}`,
  ...
  it('computes average paragraph length from the ast', () => {
    ...
    console.log(
      [
        '',
        '===== satteri average paragraph length =====',
        `runs            : ${runs}`,
